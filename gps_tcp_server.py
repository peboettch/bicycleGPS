...
def db_log_loc(event, client, data):
    if (event == 'location'):
        db = mysql.connector.connect(
            ...
            database=dbname)
        cursor = db.cursor()
        sql = "INSERT INTO location (time, imei, type, lat, lon, prec, sat) VALUES (%s, %s, %s, %s, %s, %s, %s)"
        val = (str(dt.datetime.now()), str(client), str(data['method']), str(data['latitude']), str(data['longitude']), str(data['accuracy']), str(data['nb_sat']))
        cursor.execute(sql, val)
        db.commit()
        print("commit location successful")
        cursor.close()
        db.close()
        
def db_log_inf(event, client, battery, upd_int):
    if (event == 'info'):
        db = mysql.connector.connect(
            host=dbhost,
            port=dbport,
            ...
            database=dbname)
        cursor = db.cursor()
        sql = "INSERT INTO info (time, imei, battery, upd_interv) VALUES (%s, %s, %s, %s)"
        val = (str(dt.datetime.now()), str(client), str(battery), str(upd_int))
        cursor.execute(sql, val)
        db.commit()
        print("commit info successful")
        cursor.close()
        db.close()
...
def read_incoming_packet(client, packet):
    ...
    if (protocol_name == 'login'):
        ...
        db_log_inf('info', addresses[client]['imei'], '', '')
        ...
        if (packet_list[0] == '06'): 
            ...
            db_log_inf('info', addresses[client]['imei'], int(packet_list[2], base=16), int(packet_list[4]))
...
def answer_gps(client, query):
    ...
    db_log_loc('location', addresses[client]['imei'], positions[client]['gps'])
...
# Import dotenv with API keys and DB-Info
load_dotenv()
